...
<form>
<label><b>Operator:</b></label> <input name="data[Search][user_id]" style="width: 180px;"  class="required find_user_sel2" type="hidden"> <label><b>Date:</b></label> <input name="data[Search][report_date]" type="text" class="required datepicker"> &nbsp;
...
function submitOperatorEvents(t) {
  ...
  console.log($(myform).find('input.required'));
  $(myform).find('input.required').each(function() {
    if ($(this).val() == '') {
      missingEntry = true;
      alert('You must specify a date and an operator');
    }
  });
